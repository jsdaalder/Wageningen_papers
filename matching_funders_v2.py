...
print('Loading data...')
start = time.time()
with open('funders_and_counts_extended.csv') as f:
    reader = csv.DictReader(f)
    data = [(row['Funder'], row['Number of Papers Funded']) for row in reader]
end = time.time()
print(f'Data loaded. Time taken: {end-start} seconds')

# Prepare the data for dedupe
data_dict = {idx: {"name": unidecode(name), "count": int(count)} for idx, (name, count) in enumerate(data)}
...
if os.path.exists(training_file):
    ...
    if use_existing.lower() == 'y':
        ...
        start_prepare = time.time() # Start the timer
        deduper.prepare_training(data_dict)
        end = time.time()
        print('Training data sucessfully prepared.')
...
while suggestions and not break_labeling:
    for pair in suggestions:
        ...
        print(f"Count of 'yes' labels: {counts['y']}")
        print(f"Count of 'no' labels: {counts['n']}")
        print(f"Count of 'skip' labels: {counts['skip']}")
